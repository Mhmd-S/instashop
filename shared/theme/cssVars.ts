...
// Typography `feel` → heading personality. Until now `feel` was generated by the
// model, validated, then thrown away (never emitted to CSS). It drives the display
// headings' letter-spacing, weight, and casing — applied explicitly where the
// storefront opts in (hero/section headings), never as a global override that would
// fight per-element utilities. Exposed as --t-heading-*.
export const FEEL_MAP: Record<DesignTokens['typography']['feel'], { tracking: string; weight: string; transform: string }> = {
  serif: { tracking: '-0.01em', weight: '600', transform: 'none' },
  'sans-display': { tracking: '-0.022em', weight: '700', transform: 'none' },
  geometric: { tracking: '-0.015em', weight: '600', transform: 'none' },
  humanist: { tracking: '-0.005em', weight: '600', transform: 'none' },
  'monospace-accent': { tracking: '0.04em', weight: '600', transform: 'uppercase' },
}

// Typography `scale` → a modular ratio + display/body line-heights, also previously
// generated-and-discarded. --t-scale is the ratio the art-direction presets multiply
// heading sizes by; --t-leading-display / --t-leading-body tune vertical rhythm.
export const SCALE_MAP: Record<DesignTokens['typography']['scale'], { ratio: string; leadingDisplay: string; leadingBody: string }> = {
  tight: { ratio: '1.15', leadingDisplay: '0.95', leadingBody: '1.5' },
  normal: { ratio: '1.25', leadingDisplay: '1.0', leadingBody: '1.6' },
  airy: { ratio: '1.333', leadingDisplay: '1.08', leadingBody: '1.72' },
}
...
export function tokensToCssVars(t: DesignTokens): string {
  ...
  const vars: Record<string, string> = {
    ...
    // Typography feel/scale — finally consumed (was generated + discarded). Applied
    // explicitly by the storefront's display/section headings, never globally.
    '--t-heading-tracking': FEEL_MAP[t.typography.feel].tracking,
    '--t-heading-weight': FEEL_MAP[t.typography.feel].weight,
    '--t-heading-transform': FEEL_MAP[t.typography.feel].transform,
    '--t-scale': SCALE_MAP[t.typography.scale].ratio,
    '--t-leading-display': SCALE_MAP[t.typography.scale].leadingDisplay,
    '--t-leading-body': SCALE_MAP[t.typography.scale].leadingBody,
